...
var calendar = {
	 ...
	 * 获取阳历节日
	 */
	getFestival: function(date, lunar, cMonth, nWeek, ncWeek, isChuXi) {
		//传统节日
		if (this.lunarFestival[lunar]) {
			return this.lunarFestival[lunar]
		}
		if (isChuXi) {
			return '除夕';
		}
		//母亲节、父亲节
		if (cMonth == 5 && nWeek == 2 && ncWeek == '星期日') {
			return '母亲节'
		} else if (cMonth == 6 && nWeek == 3 && ncWeek == '星期日') {
			return '父亲节'
		}
		return this.festival[date]
	},
	...
	getMonthWeek(year, month, date) {
		...
		*/
		...
		return Math.ceil((d + (w == 0 ? 0 : 6 - w)) / 7);
	},
